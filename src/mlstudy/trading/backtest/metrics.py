...
def compute_metrics(
    ...
    """
    returns = pnl_df[return_col].dropna()
    position = pnl_df[position_col]
    cumulative = pnl_df[cumulative_col].dropna()
    ...
    # Turnover
    if traded_notional_col in pnl_df.columns and gross_notional_col in pnl_df.columns:
        turnover = compute_turnover(
            pnl_df[traded_notional_col],
            pnl_df[gross_notional_col],
        )
    else:
        turnover = 0.0
