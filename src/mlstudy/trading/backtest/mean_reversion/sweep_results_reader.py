...
Usage::
    ...
    run = load_sweep_run("runs/mr_grid_v1/20240101_120000")
...
def _load_full_dir(full_dir: Path) -> list[FullScenario]:
    ...
    return [_load_full_scenario(d) for d in scenario_dirs]
...
def load_sweep_run(run_dir: str | Path) -> SweepRunData:
    ...
    for label in ("top_full", "full"):
        ...
        if full_dir.is_dir():
            full_scenarios = _load_full_dir(full_dir)
            break
