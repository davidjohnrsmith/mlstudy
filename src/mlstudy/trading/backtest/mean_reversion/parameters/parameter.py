...
class MRParameter(Enum):
    """Mean-reversion backtest parameters."""
    ...
    # take-profit (lower threshold → exit sooner → preferred)
    ...
    @classmethod
    def from_key(cls, name: str) -> "MRParameter":
        for p in cls:
            if p.key == name:
                return p
        raise ValueError(f"Unknown MR parameter {name!r}")
